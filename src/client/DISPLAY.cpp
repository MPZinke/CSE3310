...
void DISPLAY::update(PLAY play) {
    if(std::to_string(_player_number) != play.ID) {
        ...
        //TODO Set other player chip amounts.
...
std::vector<Card> DISPLAY::trade_window() {
    ...
    while(dialog.run()) {
        if(button1.get_active()) {
            cardNames.push_back(_user_cards[0]);
        }
        if(button2.get_active()) {
            cardNames.push_back(_user_cards[1]);
        }
        if(button3.get_active()) {
            cardNames.push_back(_user_cards[2]);
        }
        if(button4.get_active()) {
            cardNames.push_back(_user_cards[3]);
        }
        if(button5.get_active()) {
            cardNames.push_back(_user_cards[4]);
        }
